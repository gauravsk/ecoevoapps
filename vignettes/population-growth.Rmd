---
title: "Models of population growth"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Models of population growth}
...
The package implements various models that differ their biological interpretations and assumptions. Most of these models focus on "closed" populations, which are populations whose size only grow or shrink due to births and deaths -- immigration and emigration are not relevant. The final model in this vignette accounts for immigration in the form of populations immigrating from "source" habitats where they have high rates of population growth, to "sink" habitats where the population would not be sustained in the lack of immigration. 
...
We can plot the size of this population over time as follows:

```{r}
sim_df_exp %>%
  data.frame %>%
  ggplot(aes(x = time, y = N1)) + 
  geom_line() +
  theme_apps()
```
