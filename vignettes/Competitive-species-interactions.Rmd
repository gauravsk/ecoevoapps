...
If you are curious about how the function `run_lvcomp_model()` runs behind the scenes, read on!

Behind the scenes, `run_lvcomp_model` is using the wonderful [`deSolve`](http://desolve.r-forge.r-project.org/) package to solve the differential equations in the Lotka-Volterra model. To use this package, we first need to write a function that describes the differential equations we want to solve, and then we can use the function `deSolve::ode()` to solve the differential equation. The code is provided below, and we strongly recommend that you explore some deSolve tutorials to understand the workflow.  ([ex. 1](https://cran.r-project.org/web/packages/deSolve/vignettes/deSolve.pdf); [ex. 2](https://www.youtube.com/watch?v=dmfAgoLzhnk); there's also an [example](https://www.youtube.com/watch?v=zRlgvUuc9QY) specifically on the Lotka-Volterra predator-prey models!) 
...
init <- c(N1 = 10, N2 = 30)
time <- 0:500
params <- c(r1 = .2, r2 = .1, K1 = 500, K2 = 600,
            a = .9, b = 1.1)
