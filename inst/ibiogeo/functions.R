...
I = function(S,D,M,k){ exp(-(k/D)*(S-M))-1 } # immigration rate: number of new species arriving per year

E = function(S,A,M,k){ exp(k*S/A)-1 } # extinction rate: number of species being lost per year

S = function(D,A,M,k){ A*M/(D+A) } # number of species at equilibrium
...
mapNrates = function(D,A,M,k){
  ...
  text(x=3,y=D[1]-sqrt(A[1])*min(ty),"A",font=2,col=2)
  
  # Island B
  tx = -tx; ty = -ty
  polygon(x=sqrt(A[2])*tx+8,y=sqrt(A[2])*ty+D[2]-sqrt(A[2])*min(ty),lwd=3,border=4)
  text(x=8,y=D[2]-sqrt(A[2])*min(ty),"B",font=2,col=4)
  ...
  # Plot legend
  legend("topright",bty="n",col=c(2,4,1,1),pch=c(15,15,NA,NA),pt.cex=2,c("Island A","Island B","immigration","extinction"),lty=c(NA,NA,1,2),lwd=2)
