...
params_wo_K <- reactive({
  c(r1 = input$r1_wo_K, r2 = input$r2_wo_K,
    a11 = input$alpha11, a12 = input$alpha12,
    a22 = input$alpha22, a21 = input$alpha21)
})

# Compute equilibria
sp1_equilibria_wo_K <- reactive({
  c((input$alpha22 - input$alpha12)/(input$alpha11*input$alpha22 - input$alpha12*input$alpha21),
    0, 1/input$alpha11) %>%
    round()
})
sp2_equilibria_wo_K <- reactive({
  c((input$alpha11 - input$alpha21)/(input$alpha11*input$alpha22 - input$alpha12*input$alpha21),
    0, 1/input$alpha22) %>%
    round()
})
...
lvcomp_out_wo_K <- reactive({
  if (input$run_time_wo_K == "run_spec") {
    ode(func = lotka_volterra_competition_wo_K, y = init_wo_K(),
        times = time_wo_K(), parms = params_wo_K()) %>%
      data.frame()
    ...
    lvcomp_iter_wo_K <- 
      ode(func = lotka_volterra_competition_wo_K, y = init_wo_K(),
          times = run_seq_wo_K, parms = params_wo_K()) %>%
      data.frame()
...
N_vs_Time_wo_K <- reactive({
  if ("N_vs_Time_wo_K" %in% input$plots_to_show_wo_K) {
    ggplot(data = lvcomp_out_long_wo_K()) +
      ...
      scale_color_brewer(palette = "Set1", labels = c("1", "2")) +
      labs(x = "Time", y = "Population size (N)", color = "Species") +
      coord_cartesian(ylim = c(0, 1.1*max(c(1/input$alpha11, 1/input$alpha22,
...
ZNGI_sp1_wo_K <- reactive({
  data.frame(x1 = 0, y1 = 1/input$alpha12, xend1 = 1/input$alpha11, yend1 = 0)
})
ZNGI_sp2_wo_K <- reactive({
  data.frame(x2 = 0, y2 = 1/input$alpha22, xend2 = 1/input$alpha21, yend2 = 0)
})
  ...
  if ("ZNGI_wo_K" %in% input$plots_to_show_wo_K) {
    ggplot(data = lvcomp_out_wo_K()) +
      ...
      geom_segment(x = lvcomp_out_wo_K()$N1[round(nrow(lvcomp_out_wo_K())/4)],
                   ...
                   arrow = arrow(length = unit(0.15, "inches"), type = "open")) +
      labs(x = expression(N[1]), y = expression(N[2]), color = "ZNGI for") +
      coord_cartesian(xlim = c(0, 1.1*max(c(1/input$alpha11, input$N1_wo_K, 1/input$alpha21))),
                      ylim = c(0, 1.1*max(c(1/input$alpha22, input$N2_wo_K, 1/input$alpha12))),
                      expand = FALSE,
                      clip = "off")
  }
})
