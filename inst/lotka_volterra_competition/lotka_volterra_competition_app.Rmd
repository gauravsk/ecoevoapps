...
sidebarLayout(
  sidebarPanel(
    ...
    sliderInput(inputId = "r1", 
                label = "r1: Intrinsic growth rate of Species 1",
                min = 0.001, max = 1, value = 0.2),
    sliderInput(inputId = "r2", 
                label = "r2: Intrinsic growth rate of Species 2",
                min = 0.001, max = 1, value = 0.5),
    numericInput(inputId = "K1", 
                 label = "K1: Carrying capacity of Species 1",
                 min = 1, value = 300),
    numericInput(inputId = "K2", 
                 label = "K2: Carrying capacity of Species 2",
                 min = 1, value = 200),
    sliderInput(inputId = "alpha", 
                label = "alpha: Relative effect of Species 2 on Species 1",
                min = 0.001, max = 2, value = 0.3),
    sliderInput(inputId = "beta", 
                label = "beta: Relative effect of Species 1 on Species 2",
...
lvcomp_out <- reactive({
  if (input$run_time == "run_spec") {
    data.frame(ode(func = lotka_volterra_competition, y = init(),
    ...
    while (equilibrium == FALSE) {
      ...
      # Compute difference between last two entries of N1
      delta_N1 <- 
        round(lvcomp_iter$N1[nrow(lvcomp_iter)], 3) - 
        round(lvcomp_iter$N1[nrow(lvcomp_iter) - 1], 3)
      # Compute difference between last two entries of N2
      delta_N2 <- 
        round(lvcomp_iter$N2[nrow(lvcomp_iter)], 3) - 
        round(lvcomp_iter$N2[nrow(lvcomp_iter) - 1], 3)
      if (delta_N1 == 0 & delta_N2 == 0) {
        equilibrium <- TRUE
      } else {
        runs <- runs + 100
        run_seq <- seq(from = 0, to = runs, by = 0.1)
  ...
  if ("ZNGI" %in% input$plots_to_show) {
    ggplot(data = lvcomp_out()) +
      geom_segment(data = ZNGI_sp1(),
                   aes(x = x1, y = y1, xend = xend1, yend = yend1, color = "Species 1"),
                   size = 2) +
      geom_segment(data = ZNGI_sp2(),
                   aes(x = x2, y = y2, xend = xend2, yend = yend2, color = "Species 2"),
                   size = 2) +
      ...
      geom_segment(x = lvcomp_out()$N1[round(length(time()))/4],
                   y = lvcomp_out()$N2[round(length(time()))/4],
                   xend = lvcomp_out()$N1[round(length(time()))/4 + 1],
                   yend = lvcomp_out()$N2[round(length(time()))/4 + 1],
