...
The Pulliam model assumes the source and sink sites differ only in their effect on the fecundity ($\beta$). By definition, the source population has a positive population growth rate ($\lambda_1$ > 1), and the sink population a negative population growth rate ($\lambda$ < 1).     
...
ssServer <- function(id) {
    moduleServer(id, function(input, output, session) {
        ...
        ss_df <- reactive({ data.frame(year = 1:time(), 
                                      run_source_sink(endtime = time(), init = init(), 
                                                    params = params())) })
        ss_out <- reactive({
           out <- ss_df()
