...
shinyApp( #to embed shiny app into the html document
  ...
  ui = fluidPage(
    ...
    tabsetPanel(
      ...
      fluidRow(
          ...
          column(8, 
            # tabsetPanel("fixed",
              fixedPanel(right = 5, top = 30, width = "65%", plotOutput("simulation") ) # gives the plot
            # ), # end of first tabsetPanel (fixed plot)
            # tabsetPanel("fluid",
            #   fluidPanel( plotOutput("simulation") )
            # ), # end of the second tabsetPanel (fluid plot)
      ...
      fixedRow(
        column(12, 
              ...
              fixedPanel(right = 5, bottom = 15, width = "60%", height = 40,  
                actionButton("showAnswer", "Show/update sink equilibirum", style = "color: rgb(201, 18, 18);"),
    ...
    tabPanel("mobile devices",
        fluidRow(
          ...
          column(8, 
                 fluidRow(plotOutput("simulation_mobile")),  # gives the plot
                 fluidRow(
                   wellPanel( 
                    actionButton("showAnswer", "Show/update sink equilibirum", style = "color: rgb(201, 18, 18);"),
                    textOutput("equilibrium_mobile")
                   ) # end of wellpanel
  ...
  server = function(input, output, session) {
    ...
    output$simulation_mobile <- renderPlot(ss_plot())
    ...
    output$equilibrium_mobile <- renderText({ paste(equi(), ". (Read section 4 for details.)") })
...
<details>
  <summary>Click to show the Parameter Table </summary>
...
- currently the side bar style doesn't work due to adding the action button CANNOT

- ALTERNATIVE, use a different tab
