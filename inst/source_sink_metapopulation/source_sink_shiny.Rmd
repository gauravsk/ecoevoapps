...
ssServer <- function(id) {
    moduleServer(id, function(input, output, session) {
        ...
        params <- reactive({ c(pa = input$pa, pj = input$pj, 
                               betaSource = input$betaSource, betaSink = input$betaSink, NSource = input$NSource) })
        ...
        equi <- eventReactive(input$showAnswer, {round(-(input$pa + input$pj*input$betaSource - 1)*input$NSource/(input$pa + input$pj*input$betaSink - 1), digits = 0)})
