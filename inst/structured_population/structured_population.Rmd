...
Some populations are best modeled as a collection of individuals in distinct age or size classes ([Tenhumberg, 2010](https://www.nature.com/scitable/knowledge/library/ignoring-population-structure-can-lead-to-erroneous-15128671/)). The growth of such structured populations depends on the transition rates from one class to another. For example, the survival rates of juveniles into young  adults may be different from the survival rate of young adults into older adults. We can model the dynamics of such populations by knowing the rates of *survival* from one age (or stage) into the next, and by knowing how individuals in each age (or stage) contribute to the population by giving birth to new individuals. 

By wiggling the values of these rates we can start finding what are the transitions that impact the most the growth of the whole population (sensitivity analysis).
...
2. The largest eigenvalue of the Leslie matrix is $\lambda$, the asymptotic growth rate for the whole population. When $\lambda < 1$ the population declines to extinction, $\lambda > 1$ the grows exponentially, and when $\lambda = 1$ the population size remains constant through time).
