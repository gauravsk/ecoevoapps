...
One of the most important sets of interactions in ecological communities is the interaction between consumers and resources. For example, the interactions between a herbivore and the plants it eats, between a predator and its prey, between a parasitoid and its host, can all fit within the general category of consumer-resource interactions.  Of course, all of these do have distinct biology- for examples, predation generally means the victim dies entirely, while plants don't necessarily die when they are consumed by herbivores. Similarly parasitoids generally attack early stages in the life cycle and kill the individual before reproduction, while parasites live on or in the host and reduce the fitness of the host without killing it. See the References section below for resource on each of these different types of interactions.

Even with these biological distinctions, we can use similarly classes of models to study the dynamics of such systems. This app presents several models that have been developed to study such interactions. We refer to the resource as "prey" and the consumer as "predator". There are several variants of the general consumer-resource model that are presented here, each in a different tab. 
...
**References for Plant-herbivore dynamics**:
- [Herbivory: effects on plant abundance, distribution and population growth](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1635468/pdf/rspb20063587.pdf), 2006, by J. Maron and  E. Crone 

- [Insect Herbivores and Plant Population Dynamics](https://www.annualreviews.org/doi/abs/10.1146/annurev.en.34.010189.002531?journalCode=ento), 1989, by M.J. Crawley

**References for Predator-Prey dynamics**:
...
**References for Host-Parasite/Parasitoid dynamics**:
