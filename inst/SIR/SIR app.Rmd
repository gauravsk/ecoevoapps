...
sidebarLayout(
  ...
  sidebarPanel(
    ...
    ### Ask user for initial conditions ----
    numericInput("S0_ft", label = "Initial population size of S", min = 0, value = 100),
    numericInput("I0_ft", label = "Initial population size of I", min = 0, value = 1),
    numericInput("R0_ft", label = "Initial population size of R", min = 0, value = 0),
...
# Set the initial population sizes
init_ft <- reactive({c(S = input$S0_ft, I = input$I0_ft, R = input$R0_ft)})
# Set the parameter values
params_ft <- reactive({c(m = input$m_ft, beta = input$beta_ft, v = input$v_ft, gamma = input$gamma_ft)})
# Time over which to simulate model dynamics
...
# the SIR model using the 
# parameter estimates defined above
out_ft <- reactive({data.frame(deSolve::ode(func = SIR_ft,
                               y=init_ft(), parms=params_ft(), times = time_ft()))})

# Reshape the data so that population sizes of both 
# species are in one column, and an extra column to define
# species name. This helps with the plotting...
out_long_ft <- reactive({pivot_longer(out_ft(), c(S, I, R), "group") %>% 
  mutate(group = factor(group, levels = c("S", "I", "R")))})
...
pop_out_long_ft <- reactive({ pop_out_ft() %>%
  select(time, dS, dI, dR) %>%
  pivot_longer(c(dS, dI, dR), "group") %>%
  mutate(group = factor(group, levels = c("dS", "dI", "dR")))
})
...
# Plot abundance through time ----------
abund_plot_SIR_ft <- reactive({ggplot(out_long_ft()) + 
    geom_line(aes(x = time, y = value, color = group), size = 2) + 
    scale_color_brewer(palette = "Set1") +
    ylab("Population size") + 
    ecoevoapps::theme_apps()
...
# Plot S vs I ---------
SIplot_ft <- reactive({ggplot(out_ft()) +
    geom_segment(x = out_ft()$S[round(length(time_ft()))/25], 
                 y = out_ft()$I[round(length(time_ft()))/25], 
                 xend = out_ft()$S[round(length(time_ft())/25) + 1], 
                 yend = out_ft()$I[round(length(time_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = S, y = I), size = 2) +
    scale_color_brewer(palette = "Set1") +
    ylab("I size") +
...
# Plot I vs R --------------
RIplot_ft <- reactive({ggplot(out_ft()) +
    geom_segment(x = out_ft()$R[round(length(time_ft()))/25], 
                 y = out_ft()$I[round(length(time_ft()))/25], 
                 xend = out_ft()$R[round(length(time_ft())/25) + 1], 
                 yend = out_ft()$I[round(length(time_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
  geom_path(aes(x = R, y = I), size = 2) +
  scale_color_brewer(palette = "Set1") +
...
# Plot S vs R ------------
SRplot_ft <- reactive({ggplot(out_ft()) +
    geom_segment(x = out_ft()$S[round(length(time_ft()))/25], 
                 y = out_ft()$R[round(length(time_ft()))/25], 
                 xend = out_ft()$S[round(length(time_ft())/25) + 1], 
                 yend = out_ft()$R[round(length(time_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
  geom_path(aes(x = S, y = R), size = 2) +
  scale_color_brewer(palette = "Set1") +
  ylab("R size") +
...
sidebarLayout(
  ...
  sidebarPanel(
    ...
    ### Ask user for initial conditions ----
    numericInput("S0_sis_ft", label = "Initial population size of S", min = 0, value = 100),
    numericInput("I0_sis_ft", label = "Initial population size of I", min = 0, value = 1),
    
    ### Ask user for time to simulate ----
    numericInput("time_sis_ft", label = "Time to simulate", min = 10, value = 100)
...
# Set the parameter values
params_sis_ft <- reactive({c(m = input$m_sis_ft, beta = input$beta_sis_ft, gamma = input$gamma_sis_ft)})
# Time over which to simulate model dynamics
...
# the SIR model using the 
# parameter estimates defined above
out_sis_ft <- reactive({data.frame(deSolve::ode(func = SIS_ft,
                               y = init_sis_ft(), parms = params_sis_ft(), times = time_sis_ft()))})

# Reshape the data so that population sizes of both 
# species are in one column, and an extra column to define
# species name. This helps with the plotting...
out_long_sis_ft <- reactive({pivot_longer(out_sis_ft(), c(S, I), "group") %>% 
  mutate(group = factor(group, levels = c("S", "I")))})
...
pop_out_long_sis_ft <- reactive({ pop_out_sis_ft() %>%
  select(time, dS, dI) %>%
  pivot_longer(c(dS, dI), "group") %>%
  mutate(group = factor(group, levels = c("dS", "dI")))
})
...
# Plot abundance through time ----------
abund_plot_sis_ft <- reactive({ggplot(out_long_sis_ft()) + 
    geom_line(aes(x = time, y = value, color = group), size = 2) + 
    scale_color_brewer(palette = "Set1") +
    ylab("Population size") + 
    ecoevoapps::theme_apps()
...
# Plot S vs I ---------
SIplot_sis_ft <- reactive({ggplot(out_sis_ft()) +
    geom_segment(x = out_sis_ft()$S[round(length(time_sis_ft()))/25], 
                 y = out_sis_ft()$I[round(length(time_sis_ft()))/25], 
                 xend = out_sis_ft()$S[round(length(time_sis_ft())/25) + 1], 
                 yend = out_sis_ft()$I[round(length(time_sis_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = S, y = I), size = 2) +
    scale_color_brewer(palette = "Set1") +
    ylab("I size") +
...
sidebarLayout(
  ...
  sidebarPanel(
    ...
    ### Ask user for initial conditions ----
    numericInput("S0_seir_ft", label = "Initial population size of S", min = 0, value = 100),
    numericInput("E0_seir_ft", label = "Initial population size of E", min = 0, value = 0),
    numericInput("I0_seir_ft", label = "Initial population size of I", min = 0, value = 1),
    numericInput("R0_seir_ft", label = "Initial population size of R", min = 0, value = 0),
    
    ### Ask user for time to simulate ----
    numericInput("time_seir_ft", label = "Time to simulate", min = 10, value = 100)
...
# Set the initial population sizes
init_seir_ft <- reactive({c(S = input$S0_seir_ft, E = input$E0_seir_ft, I = input$I0_seir_ft, R = input$S0_seir_ft)})
# Set the parameter values
params_seir_ft <- reactive({c(m = input$m_seir_ft, beta = input$beta_seir_ft, a = input$a_seir_ft, gamma = input$gamma_seir_ft, v = input$v_seir_ft)})
# Time over which to simulate model dynamics
...
# the SEIR model using the 
# parameter estimates defined above
out_seir_ft <- reactive({data.frame(deSolve::ode(func = SEIR_ft,
                               y = init_seir_ft(), parms = params_seir_ft(), times = time_seir_ft()))})

# Reshape the data so that population sizes of both 
# species are in one column, and an extra column to define
# species name. This helps with the plotting...
out_long_seir_ft <- reactive({pivot_longer(out_seir_ft(), c(S, E, I, R), "group") %>% 
  mutate(group = factor(group, levels = c("S", "E", "I", "R")))})
...
pop_out_long_seir_ft <- reactive({ pop_out_seir_ft() %>%
  select(time, dS, dE, dI, dR) %>%
  pivot_longer(c(dS, dE, dI, dR), "group") %>%
  mutate(group = factor(group, levels = c("dS", "dE", "dI", "dR")))
})
...
# Plot abundance through time ----------
abund_plot_seir_ft <- reactive({ggplot(out_long_seir_ft()) + 
    geom_line(aes(x = time, y = value, color = group), size = 2) + 
    scale_color_brewer(palette = "Set1") +
    ylab("Population size") + 
    ecoevoapps::theme_apps()
...
# Plot S vs E ---------
SEplot_seir_ft <- reactive({ggplot(out_seir_ft()) +
    geom_segment(x = out_seir_ft()$S[round(length(time_seir_ft()))/25], 
                 y = out_seir_ft()$E[round(length(time_seir_ft()))/25], 
                 xend = out_seir_ft()$S[round(length(time_seir_ft())/25) + 1], 
                 yend = out_seir_ft()$E[round(length(time_seir_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = S, y = E), size = 2) +
    scale_color_brewer(palette = "Set1") +
    ylab("E size") +
...
# Plot S vs I ---------
SIplot_seir_ft <- reactive({ggplot(out_seir_ft()) +
    geom_segment(x = out_seir_ft()$S[round(length(time_seir_ft()))/25], 
                 y = out_seir_ft()$I[round(length(time_seir_ft()))/25], 
                 xend = out_seir_ft()$S[round(length(time_seir_ft())/25) + 1], 
                 yend = out_seir_ft()$I[round(length(time_seir_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = S, y = I), size = 2) +
    scale_color_brewer(palette = "Set1") +
    ylab("I size") +
...
# Plot S vs R ---------
SRplot_seir_ft <- reactive({ggplot(out_seir_ft()) +
    geom_segment(x = out_seir_ft()$S[round(length(time_seir_ft()))/25], 
                 y = out_seir_ft()$R[round(length(time_seir_ft()))/25], 
                 xend = out_seir_ft()$S[round(length(time_seir_ft())/25) + 1], 
                 yend = out_seir_ft()$R[round(length(time_seir_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = S, y = R), size = 2) +
    scale_color_brewer(palette = "Set1") +
    ylab("R size") +
...
# Plot E vs I ---------
EIplot_seir_ft <- reactive({ggplot(out_seir_ft()) +
    geom_segment(x = out_seir_ft()$E[round(length(time_seir_ft()))/25], 
                 y = out_seir_ft()$I[round(length(time_seir_ft()))/25], 
                 xend = out_seir_ft()$E[round(length(time_seir_ft())/25) + 1], 
                 yend = out_seir_ft()$I[round(length(time_seir_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = E, y = I), size = 2) +
    scale_color_brewer(palette = "Set1") +
...
# Plot E vs R ---------
ERplot_seir_ft <- reactive({ggplot(out_seir_ft()) +
    geom_segment(x = out_seir_ft()$E[round(length(time_seir_ft()))/25], 
                 y = out_seir_ft()$R[round(length(time_seir_ft()))/25], 
                 xend = out_seir_ft()$E[round(length(time_seir_ft())/25) + 1], 
                 yend = out_seir_ft()$R[round(length(time_seir_ft())/25) + 1], 
...
# Plot R vs I ---------
RIplot_seir_ft <- reactive({ggplot(out_seir_ft()) +
    geom_segment(x = out_seir_ft()$R[round(length(time_seir_ft()))/25], 
                 y = out_seir_ft()$I[round(length(time_seir_ft()))/25], 
                 xend = out_seir_ft()$R[round(length(time_seir_ft())/25) + 1], 
                 yend = out_seir_ft()$I[round(length(time_seir_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = R, y = I), size = 2) +
    scale_color_brewer(palette = "Set1") +
...
SEIR_2d_plots_ft <- reactive({
  wrap_plots(SEplot_seir_ft(), SIplot_seir_ft(), SRplot_seir_ft(), EIplot_seir_ft(), ERplot_seir_ft(), RIplot_seir_ft(), ncol = 3)
})
...
sidebarLayout(
  ...
  sidebarPanel(
    ...
    ### Ask user for initial conditions ----
    numericInput("S0_sird_ft", label = "Initial population size of S", min = 0, value = 100),
    numericInput("I0_sird_ft", label = "Initial population size of I", min = 0, value = 1),
    numericInput("R0_sird_ft", label = "Initial population size of R", min = 0, value = 0),
    numericInput("D0_sird_ft", label = "Initial population size of D", min = 0, value = 0),
    
    ### Ask user for time to simulate ----
    numericInput("time_sird_ft", label = "Time to simulate", min = 10, value = 100)
...
# Set the initial population sizes
init_sird_ft <- reactive({c(S = input$S0_sird_ft, I = input$I0_sird_ft, R = input$S0_sird_ft, D = input$D0_sird_ft)})
# Set the parameter values
params_sird_ft <- reactive({c(m = input$m_sird_ft, beta = input$beta_sird_ft, mu = input$mu_sird_ft, gamma = input$gamma_sird_ft, v = input$v_sird_ft)})
# Time over which to simulate model dynamics
...
# the sird model using the 
# parameter estimates defined above
out_sird_ft <- reactive({data.frame(deSolve::ode(func = SIRD_ft,
                               y = init_sird_ft(), parms = params_sird_ft(), times = time_sird_ft()))})

# Reshape the data so that population sizes of both 
# species are in one column, and an extra column to define
# species name. This helps with the plotting...
out_long_sird_ft <- reactive({pivot_longer(out_sird_ft(), c(S, I, R, D), "group") %>% 
  mutate(group = factor(group, levels = c("S", "I", "R", "D")))})
...
pop_out_long_sird_ft <- reactive({ pop_out_sird_ft() %>%
  select(time, dS, dI, dR, dD) %>%
  pivot_longer(c(dS, dI, dR, dD), "group") %>%
  mutate(group = factor(group, levels = c("dS", "dI", "dR", "dD")))
})
...
# Plot abundance through time ----------
abund_plot_sird_ft <- reactive({ggplot(out_long_sird_ft()) + 
    geom_line(aes(x = time, y = value, color = group), size = 2) + 
    scale_color_brewer(palette = "Set1") +
    ylab("Population size") + 
    ecoevoapps::theme_apps()
...
# Plot S vs D ---------
SDplot_sird_ft <- reactive({ggplot(out_sird_ft()) +
    geom_segment(x = out_sird_ft()$S[round(length(time_sird_ft()))/25], 
                 y = out_sird_ft()$D[round(length(time_sird_ft()))/25], 
                 xend = out_sird_ft()$S[round(length(time_sird_ft())/25) + 1], 
                 yend = out_sird_ft()$D[round(length(time_sird_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = S, y = D), size = 2) +
    scale_color_brewer(palette = "Set1") +
    ylab("D size") +
...
# Plot S vs I ---------
SIplot_sird_ft <- reactive({ggplot(out_sird_ft()) +
    geom_segment(x = out_sird_ft()$S[round(length(time_sird_ft()))/25], 
                 y = out_sird_ft()$I[round(length(time_sird_ft()))/25], 
                 xend = out_sird_ft()$S[round(length(time_sird_ft())/25) + 1], 
                 yend = out_sird_ft()$I[round(length(time_sird_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = S, y = I), size = 2) +
    scale_color_brewer(palette = "Set1") +
    ylab("I size") +
...
# Plot S vs R ---------
SRplot_sird_ft <- reactive({ggplot(out_sird_ft()) +
    geom_segment(x = out_sird_ft()$S[round(length(time_sird_ft()))/25], 
                 y = out_sird_ft()$R[round(length(time_sird_ft()))/25], 
                 xend = out_sird_ft()$S[round(length(time_sird_ft())/25) + 1], 
                 yend = out_sird_ft()$R[round(length(time_sird_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = S, y = R), size = 2) +
    scale_color_brewer(palette = "Set1") +
    ylab("R size") +
...
# Plot D vs I ---------
DIplot_sird_ft <- reactive({ggplot(out_sird_ft()) +
    geom_segment(x = out_sird_ft()$D[round(length(time_sird_ft()))/25], 
                 y = out_sird_ft()$I[round(length(time_sird_ft()))/25], 
                 xend = out_sird_ft()$D[round(length(time_sird_ft())/25) + 1], 
                 yend = out_sird_ft()$I[round(length(time_sird_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = D, y = I), size = 2) +
    scale_color_brewer(palette = "Set1") +
...
# Plot D vs R ---------
DRplot_sird_ft <- reactive({ggplot(out_sird_ft()) +
    geom_segment(x = out_sird_ft()$D[round(length(time_sird_ft()))/25], 
                 y = out_sird_ft()$R[round(length(time_sird_ft()))/25], 
                 xend = out_sird_ft()$D[round(length(time_sird_ft())/25) + 1], 
                 yend = out_sird_ft()$R[round(length(time_sird_ft())/25) + 1], 
...
# Plot R vs I ---------
RIplot_sird_ft <- reactive({ggplot(out_sird_ft()) +
    geom_segment(x = out_sird_ft()$R[round(length(time_sird_ft()))/25], 
                 y = out_sird_ft()$I[round(length(time_sird_ft()))/25], 
                 xend = out_sird_ft()$R[round(length(time_sird_ft())/25) + 1], 
                 yend = out_sird_ft()$I[round(length(time_sird_ft())/25) + 1], 
                 arrow = arrow(length = unit(0.15, "npc"))) +
    geom_path(aes(x = R, y = I), size = 2) +
    scale_color_brewer(palette = "Set1") +
...
SIRD_2d_plots_ft <- reactive({
  wrap_plots(SIplot_sird_ft(), SRplot_sird_ft(), SDplot_sird_ft(), RIplot_sird_ft(), DIplot_sird_ft(), DRplot_sird_ft(),  ncol = 3)
})
