...
# Use the SIR function above to run  the SIR model 
...
# Use the SIS function to simulate dynamics
out_sis <- reactive({
  data.frame(deSolve::ode(func = SIS, y = init_sis(),
                          parms = params_sis(), times = time_sis()))
})
...
# Set the initial population sizes
init_seir <- reactive({
  c(S = input$S0_seir, E = input$E0_seir, I = input$I0_seir, R = input$S0_seir)
})
# Set the parameter values
...
out_seir <- reactive({
  data.frame(deSolve::ode(func = SEIR, y = init_seir(), 
                          parms = params_seir(), times = time_seir()))
})
...
# Use the sird function to simulate
out_sird <- reactive({
  data.frame(deSolve::ode(func = SIRD, y = init_sird(), 
                          parms = params_sird(), times = time_sird()))
  })
...
# parameter estimates defined above
out_ft <- reactive({
  data.frame(deSolve::ode(func = SIR_ft, y=init_ft(), 
                          parms=params_ft(), times = time_ft()))
  })
...
# parameter estimates defined above
out_sis_ft <- reactive({
  data.frame(deSolve::ode(func = SIS_ft, y = init_sis_ft(), 
                          parms = params_sis_ft(), times = time_sis_ft()))
  })
...
init_seir_ft <- reactive({
  c(S = input$S0_seir_ft, E = input$E0_seir_ft, 
    I = input$I0_seir_ft, R = input$S0_seir_ft)
  })
# Set the parameter values
...
# Use the SEIR function above to simulate dynamics
out_seir_ft <- reactive({
  data.frame(deSolve::ode(func = SEIR_ft, y = init_seir_ft(), 
                          parms = params_seir_ft(), times = time_seir_ft()))
})
...
init_sird_ft <- reactive({
  c(S = input$S0_sird_ft, I = input$I0_sird_ft, R = input$S0_sird_ft, D = 0)
  })
# Set the parameter values
...
out_sird_ft <- reactive({
  data.frame(deSolve::ode(func = SIRD_ft, y = init_sird_ft(), 
                          parms = params_sird_ft(), times = time_sird_ft()))
})
