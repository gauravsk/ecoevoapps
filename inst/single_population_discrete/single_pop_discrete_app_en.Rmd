...
exponential_pop_df <- reactive({
  run_discrete_exponential(N0 = input$N0, lambda = input$lambda, time = input$time)
})


# Make plot -----------
trajaectory_plot <- reactive({
  ggplot(exponential_pop_df()) +
...
dl_df <- reactive({
  run_discrete_logistic_model(N0 = input$N0_dl, K = input$K_dl,
...
cobweb_dl <- reactive({
  dl_df2 <- dl_df()
  dl_df2$Ntm1 <- dplyr::lag(dl_df2$Nt)
  ggplot(dl_df2) +
