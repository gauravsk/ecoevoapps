...
cobweb_dl <- reactive({
  plot_discrete_population_cobweb(dl_df(),
                                  params_vec = c(rd = input$rd_dl, K = input$K_dl),
...
ricker_df <- reactive({
  ricker_out <- run_ricker_model(N0 = input$N0_ricker, K = input$K_ricker,
                              rd = input$r_ricker, time = input$time_ricker)
...
cobweb_ricker <- reactive({
    ...
    plot_discrete_population_cobweb(ricker_df(),
                                  params_vec = c(rd = input$r_ricker, K = input$K_ricker),
                                  model_type = "ricker")
})
...
# Get user defined parameters for discrete logistic  ------
bh_df <- reactive({
  run_beverton_holt_model(N0 = input$N0_bh, K = input$K_bh, 
