...
***[中文](https://ecoevoapps.shinyapps.io/single_pop_discrete_ch/) | [Español](https://ecoevoapps.shinyapps.io/single_pop_discrete_es/) | English ***
...
dl_df <- reactive({
  dl_df <- run_discrete_logistic_model(N0 = input$N0_dl, K = input$K_dl,
                              rd = input$rd_dl, time = input$time_dl)
  dl_df$Ntm1 <- dplyr::lag(dl_df$Nt)
  dl_df
...
cobweb_dl <- reactive({
  ggplot(dl_df()) +
    stat_function(fun = function(x) ecoevoapps:::discretelogistic_eqn(x, input$rd_dl, input$K_dl),
...
ricker_df <- reactive({
  ricker_out <- run_ricker_model(N0 = input$N0_ricker, K = input$K_ricker,
                              r = input$r_ricker, time = input$time_ricker)
  
  ricker_out$Ntm1 <- dplyr::lag(ricker_out$Nt)
  ricker_out
})
# Generate trajectories for discrete logistic --------
...
cobweb_ricker <- reactive({
  ggplot(ricker_df()) + 
    stat_function(fun = function(x) ecoevoapps:::ricker_eqn(x, input$r_ricker, input$K_ricker),
...
# Get user defined parameters for discrete logistic  ------
bh_df <- reactive({
  bv_df <- run_beverton_holt_model(N0 = input$N0_bh, K = input$K_bh, 
                                   Rd = input$r_bh, time = input$time_bh)
  bv_df$Ntm1 <- dplyr::lag(bv_df$Nt)
  bv_df
})
...
cobweb_bh <- reactive({
  ggplot(bh_df()) + 
    stat_function(fun = function(x) ecoevoapps:::bevertonholt_eqn(x, input$r_bh, input$K_bh),
